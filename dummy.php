...
<script>
    ...
    .then(response => response.json())
    .then(data => {
        const objet2Select = document.getElementById('objet2');
        objet2Select.innerHTML = '';

        data.forEach(objet => {
            const option = document.createElement('option');
            option.value = objet.name;
            option.text = objet.name;
            objet2Select.appendChild(option);
        });
    })
    .catch(error => console.error('Error fetching objet2 data:', error));

    // Fetch remarque2 data
    fetch('API.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
    ...
    .then(response => response.json())
    .then(data => {
        const remarque2Select = document.getElementById('remarque2');
        remarque2Select.innerHTML = '';

        data.forEach(remarque => {
            const option = document.createElement('option');
            option.value = remarque.name;
            option.text = remarque.name;
            remarque2Select.appendChild(option);
        });
    })
